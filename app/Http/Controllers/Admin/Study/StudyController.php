...
class StudyController extends GlobalController
{
    ...
    **/
    ...
    /**
        * Select clinical ward location
        *
        * @param $id
        *
        * @return to add & edit study page on cr location click append related clinical ward location through Ajax call
    **/
    public function selectClinicalWardLocation(Request $request){

        $clinicalWordLocation = ClinicalWardMaster::where('location_id', $request->id)->get();

        return $clinicalWordLocation;
    }
    ...
    **/
    public function saveStudy(Request $request){
        ...
        $project->clinical_word_location = $request->clinical_word_location;
        ...
        $projectTrail->clinical_word_location = $request->clinical_word_location;
    ...
    public function editStudy($id){
        ...
        $study = Study::where('id', base64_decode($id))
                      ->with([
                            ...
                            'studyRegulatory',
                            'drugDetails' => function($q){
                                $q->select('id', 'study_id','drug_id', 'dosage_form_id', 'dosage', 'drug_strength', 'uom_id', 'type', 'manufacturedby', 'remarks');
                            },
                        ])
        ...
        $clinicalWordLocation = ClinicalWardMaster::select('id', 'ward_name')
    ...
    **/
    public function selectDrugDetails(Request $request){
        ...
        $html .= '</select><span class="select_type_error" style="color: red; display: none;">Please select drug reference</span></td><td><input type="text" class="form-control manufacture noValidate" name="drug['.$request->id.'][manufacture]" id="manufacture" placeholder="Manufacture / Distribution By" autocomplete="off" required/><span class="manufacture_error" style="color: red; display: none;">Please enter manufacture</span></td><td><input type="text" class="form-control remark noValidate" name="drug['.$request->id.'][remark]" id="remark" placeholder="Remark" autocomplete="off" required/><span class="remark_error" style="color: red; display: none;">Please enter remark</span></td><br><td><a href="javascript:void(0);" class="text-danger remove" data-toggle="tooltip" data-id="'.$request->id.'" data-placement="top" title="" data-original-title="Remove"><i class="mdi mdi-close font-size-20"></i></a></td></tr>';
    ...
    public function copyStudy($id){
        ...
        if($checkGroup < $originalStudy->no_of_groups){
            ...
            $studyTrail->clinical_word_location = $originalStudy->clinical_word_location;
